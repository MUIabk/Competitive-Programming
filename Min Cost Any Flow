...
public:
    ...
    pair<Cap, Cost> flow() { return flow(inf<Cap>); }
    pair<Cap, Cost> flow(Cap flow_limit) { return slope(flow_limit).back(); }
    vector<pair<Cap, Cost>> slope() { return slope(inf<Cap>); }
...
private:
    ...
    vector<pair<Cap, Cost>> slope(internal::csr<_edge>& g, Cap flow_limit) {
        ...
        vector<Q> que;
        auto dual_ref = [&]() {
            for (int i = 0; i < n; i++) { dual_dist[i].second = inf<Cost>; }
